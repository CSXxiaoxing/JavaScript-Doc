...
（一）使用 Array 构造函数：
（二）使用数组字面量表示法：
`join()  /dʒɔɪn/`  `push() 和 pop()`  `shift() 和 unshift()`  `sort()    /sɔrt`  `reverse()    /rɪ'vɝs/`  `concat()`  `slice()`  `splice()`  `indexOf()和 lastIndexOf()`  `forEach()`  `map()`  `filter()`  `every()`
`some()`  `reduce()和 reduceRight()`  `find()`
